...
THREEFAB.MaterialView = Backbone.View.extend({
  ...
  render: function() {
    ...
    this.texture.render( null, this.selected );
  ...
  meshChanged: function( event, object ) {
  ...
  addMaterialOptions: function() {
    if (this.selected.material) {
      ...
      this.folders.materials.add(this.selected.material, 'name', {Basic: 'MeshBasicMaterial', Phong:'MeshPhongMaterial', Lambert: 'MeshLambertMaterial'}).onChange( this.rebuildMaterial ).name('shader');
  ...
  changeColor: function( event, c, type ) {
  ...
  changeLightColor: function( event, c ) {
